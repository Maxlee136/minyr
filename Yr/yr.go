...
func Convert() error {
	...
	if _, err := os.Stat("yr/kjevik-temp-fahr-20220318-20230318.csv"); !os.IsNotExist(err) {
		...
		fmt.Print("filen eksisterer fra før av, ønsker du å lage den på nytt? (y/n): ")
	...
	if err != nil {
		fmt.Println("åpning av input fil, sjekk om filen er i riktig mappe:", err)
	}
	...
	dataText := "Data er basert paa gyldig data (per 18.03.2023) (CC BY 4.0) fra Meteorologisk institutt (MET); endringen er gjort av Max lee"
